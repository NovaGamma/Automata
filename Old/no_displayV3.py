...
class Node():
    ...
    def __next__(self):
        if self.i >= len(self.transitions):
            raise StopIteration
        else:
            self.i += 1
            return self.transitions[self.i-1]
...
class Automaton():
    ...
    def determinize(self,previous_table = 'None', det_table = 'None'):
        if det_table == 'None':
            det_table = {}
        if previous_table == 'None':
            previous_table = self.real_table.copy()
        for key in previous_table.copy().keys():#here we go through all the states
            i = previous_table[key]#i is the list of transition
            for j in i:#j is a transition of the row i
                if len(j.split(' , '))>1:#mean that we need a composite state
                    states = j.split(' , ')
                    if "".join(states) in previous_table.keys():
                        index = i.index(j)
                        previous_table[key][index] = "".join(states)
                        return previous_table
                    row = [' ' for i in range(len(self.alphabet))]
                    print(states)
                    for k in states:
                        transitions = previous_table[int(k)]#here are the transition of a component of the new state
                    ...
                    for m in row:
                        if len(m.split(' , '))>1:
                            self.determinize(previous_table,det_table)
                            return det_table
